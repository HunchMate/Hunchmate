          ...
          <div className="home-featured__grid">
            {featuredEvents.map((event, i) => (
              <div 
                ...
                style={{ height: '100%' }}
                onClick={() => navigate(`/events/${event.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}`)}
              >
                <EventCard 
                  coverImage={event.showcaseImage || event.posterImage || ''}
                  title={event.title}
                  categories={[
                  ...
                  startDate={event.timeline?.eventStart || event.startDate}
                  endDate={event.timeline?.eventEnd || event.endDate}
                  ...
                  onRegister={() => navigate(`/events/${event.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}`)}
                  onBookmark={() => {}}
                />
              </div>
            ))}
